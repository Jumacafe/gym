...
function rCal(workout){
 ...
 for(var i=1;i<=days;i++){
  ...
  var dayTpl=templatesForDate(k,data.templates||[]);
  var tplBadge=dayTpl.length?'<div style="position:absolute;bottom:1px;left:50%;transform:translateX(-50%);font-size:7px;color:#00D084;font-weight:700;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;max-width:32px;text-align:center;line-height:1">'+dayTpl[0].name.substring(0,4)+'</div>':'';
  cells+='<button class="cd'+(isSel?' sel':'')+(isTod?' tod':'')+(dayTpl.length?' with-tpl':'')+'" onclick="selD(\''+k+'\')" style="position:relative">'+i+(has?'<div class="dot"></div>':'')+tplBadge+'</button>';
 }
