...
function rDayRecommendation(){
 var scheduled=S.data.scheduledRoutines||{};
 var templates=S.data.templates||[];
 if(!templates.length)return '';

 // Calcular índice del día actual (lunes=0, igual que getFDW y rConfig)
 var todayJs=new Date().getDay();
 var todayIdx=todayJs===0?6:todayJs-1;
 var tplId=scheduled[todayIdx];
 if(!tplId)return '';

 var tpl=templates.find(function(t){return t.id===tplId;});
 if(!tpl)return '';

 var clr=tpl.color||'#FF3B3B';
 var dayNames=['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'];
 var tags=tpl.exercises.map(function(e){
  return '<span class="etag" style="border-color:'+clr+';color:'+clr+'">'+e.name+'</span>';
 }).join('');

 return '<div style="background:linear-gradient(135deg,#1a0a0a 0%,#140808 100%);border:1px solid '+clr+';border-left:4px solid '+clr+';border-radius:12px;padding:14px;margin-bottom:12px;">'+
  '<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">'+
  '<span style="font-size:10px;font-family:\'Barlow Condensed\',sans-serif;font-weight:800;letter-spacing:1px;color:'+clr+';background:'+clr+'22;padding:3px 8px;border-radius:4px;">HOY · '+dayNames[todayIdx].toUpperCase()+'</span>'+
  '<span style="font-size:10px;color:#555;">Rutina recomendada</span>'+
  '</div>'+
  '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">'+
  '<div>'+
  '<div style="font-family:\'Barlow Condensed\',sans-serif;font-weight:800;font-size:20px;color:#fff;letter-spacing:0.5px;">'+tpl.name+'</div>'+
  '<div style="font-size:11px;color:#555;margin-top:1px;">'+tpl.exercises.length+' ejercicios</div>'+
  '</div>'+
  '<button class="bp" style="padding:7px 14px;font-size:12px;background:'+clr+';border:none;" onclick="applyTFromCal(\''+tpl.id+'\')">'+IC.check+' Aplicar</button>'+
  '</div>'+
  '<div>'+tags+'</div>'+
  '</div>';
}
