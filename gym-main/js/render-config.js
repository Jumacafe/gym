// ── RUTINAS PROGRAMADAS POR DÍA ──
function rScheduledRoutinesConfig(){
 var scheduled=S.data.scheduledRoutines||{};
 var templates=S.data.templates||[];
 var dayNames=['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo'];
 // Obtener índice de día actual (lunes=0)
 var todayJs=new Date().getDay();
 var todayIdx=todayJs===0?6:todayJs-1;

 if(templates.length===0){
  return '<div class="ct" style="color:#FF3B3B;margin-bottom:6px;font-size:11px">RUTINAS POR DÍA</div>'+
   '<div class="card" style="margin-bottom:14px">'+
   '<div style="font-size:12px;color:#555;text-align:center;padding:8px 0">Crea al menos una rutina para poder programarla por día</div>'+
   '</div>';
 }

 var rows=dayNames.map(function(name,i){
  var assignedId=scheduled[i]||'';
  var isToday=i===todayIdx;
  var opts='<option value="">Sin rutina</option>'+
   templates.map(function(t){
    return '<option value="'+t.id+'"'+(assignedId===t.id?' selected':'')+'>'+t.name+'</option>';
   }).join('');
  return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid #1e1e1e;">'+
   '<div style="width:80px;font-size:13px;font-family:\'Barlow Condensed\',sans-serif;font-weight:700;color:'+(isToday?'#FF3B3B':'#aaa')+';">'+
   name+(isToday?' <span style="font-size:10px;font-weight:400;color:#FF3B3B">HOY</span>':'')+
   '</div>'+
   '<select onchange="saveScheduledRoutine('+i+',this.value)" style="flex:1;background:#1a1a1a;border:1px solid #2a2a2a;border-radius:8px;padding:7px 10px;color:#eee;font-size:13px;outline:none;">'+
   opts+
   '</select>'+
   '</div>';
 }).join('');
 ...
 return '<div class="ct" style="color:#FF3B3B;margin-bottom:6px;font-size:11px">RUTINAS POR DÍA</div>'+
  '<div class="card" style="margin-bottom:14px">'+
  '<div style="font-size:11px;color:#555;margin-bottom:10px;line-height:1.5">Asigna una rutina a cada día. Verás una recomendación destacada al abrir la app.</div>'+
  rows+
  '</div>';
}
